    ...
    <el-dialog :visible.sync="dialogFormVisible" :close-on-click-modal="false" @close="cancel()"
      ...
      <el-form ref="form" :model="form" :rules="rules" size="medium" label-width="100px">
        <el-form-item label="模板名称" prop="name">
          <el-input v-model="form.name" placeholder="请输入模板名称" clearable :style="{width: '100%'}"></el-input>
        </el-form-item>
        <el-form-item label="模板类型" prop="tempType">
          <el-input-number v-model="form.tempType" placeholder="模板类型"></el-input-number>
        </el-form-item>
        <el-form-item label="默认" prop="isDefault">
          <el-input-number v-model="form.isDefault" placeholder="默认"></el-input-number>
        </el-form-item>
        <el-form-item label="状态" prop="status">
          <el-input-number v-model="form.status" placeholder="状态"></el-input-number>
        </el-form-item>
        <el-form-item label="排序" prop="sort">
          <el-input-number v-model="form.sort" placeholder="排序"></el-input-number>
        </el-form-item>
        <el-form-item label="模板内容" prop="content">
          <el-input v-model="form.content" placeholder="请输入模板内容" :style="{width: '100%'}"></el-input>
        </el-form-item>
...
export default {
  ...
  methods: {
    getList() {
      ...
      this.$axios.gets('/api/business/print-template', this.listQuery).then(response => {
    ...
    fetchData(id) {
      this.$axios.gets('/api/business/print-template/' + id).then(response => {
        this.form = response;
      });
    },
    handleFilter() {
    ...
    handleDelete(row) {
      ...
      this.$confirm('是否删除' + alert + '?', '提示', {
        ...
        this.$axios.posts('/api/business/print-template/delete', params).then(response => {
          this.$notify({
            title: '成功',
            message: '删除成功',
            type: 'success',
    ...
    save() {
      this.$refs.form.validate(valid => {
        if (valid) {
          ...
          if (this.isEdit) {
            this.$axios.posts('/api/business/print-template/data-post', this.form).then(response => {
